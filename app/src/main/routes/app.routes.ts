...
const runInTransaction = async <T>(handler: () => Promise<T>) => {
...
const buildUserView = (user: UserRow) => {
...
const loadCurrentUser = async () => {
  ...
  return { row: user as UserRow, view: buildUserView(user as UserRow) };
};

const mapDeckStatsToView = (deck: any): DeckView => ({
  id: deck.id,
  name: deck.name,
...
const mapFlashcardToView = (card: any): FlashcardView => ({
  id: card.id,
...
const formatHistory = (historyRows: any[]) =>
  historyRows.map((row) => {
...
const formatFocus = (decks: DeckView[]) => {
...
const formatDueIn = (date: Date | string | null) => {
  if (!date) {
    return "agora";
  }
...
const handleRenderError = (res: Response, error: unknown) => {
  console.error(error);
...
appRouter.get("/decks", async (_: Request, res: Response) => {
  try {
...
appRouter.get("/decks/:deckId/cards", async (req: Request, res: Response) => {
  try {
...
appRouter.get("/decks/:deckId/import", async (req: Request, res: Response) => {
  try {
    const { deckId } = req.params;
    const data = await runInTransaction(async () => {
      const { row: userRow, view: user } = await loadCurrentUser();
...
appRouter.get("/review", async (_: Request, res: Response) => {
  try {
    const data = await runInTransaction(async () => {
      const { row: userRow, view: user } = await loadCurrentUser();
...
appRouter.get("/progress", async (_: Request, res: Response) => {
  try {
    const data = await runInTransaction(async () => {
      const { row: userRow, view: user } = await loadCurrentUser();
      const deckStats = await deckModel.findDecksWithStats({ userId: userRow.id });
...
appRouter.get("/account", async (_: Request, res: Response) => {
  try {
    const data = await runInTransaction(async () => {
      const { row: userRow, view: user } = await loadCurrentUser();
